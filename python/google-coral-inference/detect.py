...
def get_output(interpreter, score_threshold, image_scale=(1.0, 1.0)):
    ...
    # EDIT begin
    # the indexes of the tensors have been reordered
    # (0, 1, 2, 3) => (1, 3, 0, 2)
    boxes = output_tensor(interpreter, 1)
    class_ids = output_tensor(interpreter, 3)
    scores = output_tensor(interpreter, 0)
    count = int(output_tensor(interpreter, 2))
    # EDIT end
